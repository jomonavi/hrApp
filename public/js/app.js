...
app.controller('EmployeesCtrl', ['$scope', 'EmployeeService', function($scope, service) {
	console.log("I HIT IT BUT FUCKED UP");
  	service.query(function(data, headers) {
    	$scope.employees = data;
    	console.log("EMPLOYEES ARRAY", $scope.employees);
  	}, _handleError);
}]);

app.controller('EmployeeCtrl', ['$scope', '$routeParams', 'EmployeeService', 'TeamService', '$q', 'config', '$route', '$http', function($scope, $routeParams, EmployeeService, team, $q, config, $route, $http) {
	$scope.address = {};
	// $scope.employee = {};
	...
	$http.get('/employees/' + $routeParams.employeeId).then(function(data){
